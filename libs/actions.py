...
from libs import api, tools, loger, check
...
def tx_status_multi(url, sleep_time, hshs, jvt_token):
    ...
    while sec < sleep_time:
        ...
        for status in jresp.values():
            if not (len(status['blockid']) > 0 and 'errmsg' not in json.dumps(status)):
                allTxInBlocks = False
                counter -= 1
        print("{} out of {}".format(counter, len(hshs)))
        if allTxInBlocks:
            return jresp
        else:
            sec = sec + 1
...
def imp_app(app_name, url, pr_key, token):
    ...
    #print(res_import_upload)
    if len(res_import_upload['blockid']) > 0:
...
def edit_platform_param(name, val, url, pr_key, token, wait):
    log.info('EditPlatformParam started')
    data = {'Name': name, 'Value': val, 'Conditions': 'true'}
    call = call_contract(url, pr_key, 'UpdateSysParam',
                                 data, token)

    # 1 - minimal possible blockId
    if check.is_tx_in_block(url, wait, {'hash': call}, token) < 1:
        log.error('EditPlatformParam ' + name + ' is failed')
        exit(1)
